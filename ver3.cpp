...
void gotoxy_clear(int x, int y) { //moves cursor to x, y then clears that line
	gotoxy(x, y);
	std::cout << "\033[K";
}

void push_output(const std::string& output) { //for output printing
	std::unique_lock<std::mutex> lock(output_queue_mutex);
    std::istringstream iss(output);
    std::string line;
    
    //splits multi-line outputs
    while (std::getline(iss, line)) {
        output_queue.push(line);  // push each line individually
    }
}
    ...
    while (is_running) {
        ...
        { // for displaying outputs
        	...
        	if (!output_queue.empty()) {
                ...
                for (int r = 0; r <= 7; r++) { //outputs have max of 7 rows
                    gotoxy_clear(0, 30 + r); //clear for outputs
                }

                int row = 30;
                while (!output_queue.empty() && row <= 37) { //outputs print from rows 30 to 37
                    gotoxy_clear(0,row);
					std::string out = output_queue.front();
                    output_queue.pop();

                    //split into lines if output is multi-line
                    gotoxy(0, row);
...
void command_process(const std::string& command_line) {
    ...
    if(cmd == "help"){
        push_output("Available commands:\n"
        ...
        if (iss >> speed && speed > 0) {
            ...
            push_output("Invalid speed.\n");
        }
    } else if (cmd == "exit") {
        is_running = false;
    } else {
        push_output("Unknown command. Type 'help' for a list of commands.\n");
    }
}
